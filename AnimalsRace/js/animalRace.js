...
function setupRace() {
    ...
    for (let k in runners) {
        ...
        let key = k; //"dog" | "duck" | "horse" | "chick";
        let animal = runners[key];
        const image = document.createElement("img");
        image.src = `images/${animal.img}`;
        image.classList.add("col", "image-fluid");
        image.id = animal.id;
        animals.push(new Animal(image, animal.name, animal.id, animal.voice, animal.img, animal.step, false, 0));
    }
    animals
